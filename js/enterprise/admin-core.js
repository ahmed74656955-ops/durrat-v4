...
/* Durrat V18.2 - Safe Maintenance Center */
(function(){
  const A=window.AdminApp || (typeof AdminApp!=='undefined'?AdminApp:null);
  if(!A || A.__v182Maintenance) return;
  A.__v182Maintenance=true;
  const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const cutoff=days=>new Date(Date.now()-Number(days)*86400000).toISOString();
  A._maintConfirm=function(message,phrase='تأكيد الصيانة'){
    const v=prompt(message+'\n\nاكتب العبارة التالية للمتابعة:\n'+phrase);
    return v===phrase;
  };
  A._maintBackup=async function(label){
    try{
      const s=this.getSession?.()||{};
      const payload={label,created_at:new Date().toISOString(),created_by:s?.user?.username||'admin',note:'نسخة وصفية قبل عملية صيانة. النسخ الفعلية للبوالص تبقى في جدول waybills.'};
      const {error}=await supabase.from('system_backups').insert([payload]);
      if(error) console.warn(error);
    }catch(e){console.warn(e)}
  };
  A.renderMaintenanceCenter=async function(area){
    let stats={logs:0,notes:0,waybills:0,users:0,branches:0};
    const count=async(table,filter)=>{try{let q=supabase.from(table).select('*',{count:'exact',head:true});if(filter)q=filter(q);const r=await q;return r.count||0}catch{return 0}};
    [stats.logs,stats.notes,stats.waybills,stats.users,stats.branches]=await Promise.all([
      count('audit_logs'),count('system_notifications'),count('waybills'),count('app_users'),count('branches')
    ]);
    area.innerHTML=`<div class="card glass-panel maintenance-center">
      <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap">
        <div><h3 style="margin:0"><i class="fas fa-screwdriver-wrench" style="color:var(--warning)"></i> مركز الصيانة وتنظيف البيانات</h3><p style="color:var(--text-muted);line-height:1.9;max-width:900px">أدوات آمنة للإدارة العليا. الإيقاف والأرشفة أفضل من الحذف النهائي. البوالص لا تُحذف تلقائيًا، وكل عملية صيانة تُسجل في سجل العمليات.</p></div>
        <button class="btn btn-info" onclick="AdminApp.showSection('maintenance_center')"><i class="fas fa-rotate"></i> تحديث الأرقام</button>
      </div>
      <div class="v18-kpi-grid" style="margin-top:18px">
        <div class="v18-kpi"><small>سجلات العمليات</small><strong>${stats.logs}</strong></div>
        <div class="v18-kpi"><small>الإشعارات</small><strong>${stats.notes}</strong></div>
        <div class="v18-kpi"><small>البوالص</small><strong>${stats.waybills}</strong></div>
        <div class="v18-kpi"><small>المستخدمون</small><strong>${stats.users}</strong></div>
        <div class="v18-kpi"><small>الفروع</small><strong>${stats.branches}</strong></div>
      </div>
      <div class="maintenance-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:16px;margin-top:20px">
        <section class="glass-panel" style="padding:18px;border-radius:18px"><h4><i class="fas fa-clock-rotate-left"></i> تنظيف سجل العمليات</h4><p style="color:var(--text-muted)">حذف السجلات القديمة فقط مع إبقاء النشاط الحديث.</p><select id="maint_audit_days" class="form-control"><option value="30">أقدم من 30 يومًا</option><option value="60">أقدم من 60 يومًا</option><option value="90" selected>أقدم من 90 يومًا</option><option value="180">أقدم من 180 يومًا</option></select><button class="btn btn-warning" style="margin-top:12px;width:100%" onclick="AdminApp.maintenanceCleanAudit(this)"><i class="fas fa-broom"></i> تنظيف السجل القديم</button></section>
        <section class="glass-panel" style="padding:18px;border-radius:18px"><h4><i class="fas fa-bell-slash"></i> تنظيف الإشعارات</h4><p style="color:var(--text-muted)">حذف الإشعارات القديمة غير اللازمة.</p><select id="maint_notif_days" class="form-control"><option value="30" selected>أقدم من 30 يومًا</option><option value="60">أقدم من 60 يومًا</option><option value="90">أقدم من 90 يومًا</option></select><button class="btn btn-warning" style="margin-top:12px;width:100%" onclick="AdminApp.maintenanceCleanNotifications(this)"><i class="fas fa-trash-clock"></i> تنظيف الإشعارات</button></section>
        <section class="glass-panel" style="padding:18px;border-radius:18px"><h4><i class="fas fa-user-slash"></i> إدارة مستخدم</h4><p style="color:var(--text-muted)">إيقاف أو أرشفة مستخدم مع الاحتفاظ ببوالصه وسجله.</p><input id="maint_username" class="form-control" placeholder="اسم المستخدم بالضبط"><div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:12px"><button class="btn btn-warning" onclick="AdminApp.maintenanceUser('suspend',this)">إيقاف</button><button class="btn btn-info" onclick="AdminApp.maintenanceUser('archive',this)">أرشفة</button><button class="btn btn-primary" onclick="AdminApp.maintenanceUser('activate',this)">إعادة تفعيل</button></div></section>
        <section class="glass-panel" style="padding:18px;border-radius:18px"><h4><i class="fas fa-building-circle-xmark"></i> إدارة فرع</h4><p style="color:var(--text-muted)">إيقاف أو إعادة تفعيل الفرع دون حذف أعماله.</p><input id="maint_branch" class="form-control" placeholder="كود الفرع مثل JA"><div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:12px"><button class="btn btn-warning" onclick="AdminApp.maintenanceBranch(false,this)">إيقاف الفرع</button><button class="btn btn-primary" onclick="AdminApp.maintenanceBranch(true,this)">تفعيل الفرع</button></div></section>
        <section class="glass-panel" style="padding:18px;border-radius:18px"><h4><i class="fas fa-mobile-screen-button"></i> تصفير الواجهة المحلية</h4><p style="color:var(--text-muted)">يمسح الكاش وتفضيلات العرض على هذا الجهاز فقط، ولا يمس قاعدة البيانات أو الأرشيف.</p><button class="btn btn-info" style="width:100%" onclick="AdminApp.maintenanceResetLocal()"><i class="fas fa-rotate-left"></i> تصفير إعدادات الجهاز</button></section>
        <section class="glass-panel" style="padding:18px;border-radius:18px;border-color:rgba(239,68,68,.45)"><h4 style="color:var(--danger)"><i class="fas fa-triangle-exclamation"></i> منطقة حساسة</h4><p style="color:var(--text-muted)">الحذف النهائي للبوالص والفروع والمستخدمين غير مفعّل من هذه الصفحة حمايةً للتاريخ التشغيلي. استخدم طلبات الحذف أو الأرشفة.</p><button class="btn btn-danger" style="width:100%" disabled><i class="fas fa-lock"></i> الحذف النهائي مقفل</button></section>
      </div>
    </div>`;
  };
  A.maintenanceCleanAudit=async function(btn){const days=Number(document.getElementById('maint_audit_days')?.value||90);if(!this._maintConfirm(`سيتم حذف سجلات العمليات الأقدم من ${days} يومًا.`))return;this.setBusy(btn,true,'جاري التنظيف...');try{await this._maintBackup('قبل تنظيف سجل العمليات');const {error}=await supabase.from('audit_logs').delete().lt('created_at',cutoff(days));if(error)throw error;await this.logAction?.('maintenance_cleanup','audit_logs',null,{older_than_days:days});this.toast('تم تنظيف سجل العمليات القديم','success');this.showSection('maintenance_center')}catch(e){this.toast('فشل التنظيف: '+e.message,'error')}finally{this.setBusy(btn,false)}};
  A.maintenanceCleanNotifications=async function(btn){const days=Number(document.getElementById('maint_notif_days')?.value||30);if(!this._maintConfirm(`سيتم حذف الإشعارات الأقدم من ${days} يومًا.`))return;this.setBusy(btn,true,'جاري التنظيف...');try{await this._maintBackup('قبل تنظيف الإشعارات');let r=await supabase.from('system_notifications').delete().lt('created_at',cutoff(days));if(r.error){r=await supabase.from('system_notes').delete().lt('created_at',cutoff(days))}if(r.error)throw r.error;await this.logAction?.('maintenance_cleanup','notifications',null,{older_than_days:days});this.toast('تم تنظيف الإشعارات القديمة','success');this.showSection('maintenance_center')}catch(e){this.toast('فشل التنظيف: '+e.message,'error')}finally{this.setBusy(btn,false)}};
  A.maintenanceUser=async function(mode,btn){const username=document.getElementById('maint_username')?.value.trim();if(!username)return this.toast('اكتب اسم المستخدم','warning');const label={suspend:'إيقاف',archive:'أرشفة',activate:'إعادة تفعيل'}[mode];if(!this._maintConfirm(`${label} المستخدم: ${username}`))return;this.setBusy(btn,true,'جاري التنفيذ...');try{const patch=mode==='activate'?{is_active:true,status:'active'}:mode==='archive'?{is_active:false,status:'archived'}:{is_active:false,status:'suspended'};let r=await supabase.from('app_users').update(patch).eq('username',username);if(r.error){delete patch.is_active;r=await supabase.from('app_users').update(patch).eq('username',username)}if(r.error)throw r.error;await this.logAction?.('maintenance_user_'+mode,'app_users',username,patch);this.toast(`تم ${label} المستخدم`,'success');this.showSection('maintenance_center')}catch(e){this.toast('تعذر تنفيذ العملية: '+e.message,'error')}finally{this.setBusy(btn,false)}};
  A.maintenanceBranch=async function(active,btn){const code=document.getElementById('maint_branch')?.value.trim().toUpperCase();if(!code)return this.toast('اكتب كود الفرع','warning');if(!this._maintConfirm(`${active?'تفعيل':'إيقاف'} الفرع: ${code}`))return;this.setBusy(btn,true,'جاري التنفيذ...');try{let r=await supabase.from('branches').update({is_active:active,status:active?'active':'suspended'}).eq('code',code);if(r.error)r=await supabase.from('branches').update({is_active:active}).eq('code',code);if(r.error)throw r.error;await this.logAction?.('maintenance_branch_status','branches',code,{is_active:active});this.toast(active?'تم تفعيل الفرع':'تم إيقاف الفرع','success');this.showSection('maintenance_center')}catch(e){this.toast('تعذر تنفيذ العملية: '+e.message,'error')}finally{this.setBusy(btn,false)}};
  A.maintenanceResetLocal=function(){if(!this._maintConfirm('سيتم حذف تفضيلات العرض والكاش من هذا الجهاز فقط.','تصفير الواجهة'))return;const keep=['enterprise_session','waybill_active_user','waybill_branch_prefix','waybill_branch_title'];const saved={};keep.forEach(k=>saved[k]=localStorage.getItem(k));Object.keys(localStorage).filter(k=>/theme|palette|filter|draft|zoom|sidebar|mobile|cache|ui_/i.test(k)).forEach(k=>localStorage.removeItem(k));Object.entries(saved).forEach(([k,v])=>{if(v!=null)localStorage.setItem(k,v)});if('caches'in window)caches.keys().then(keys=>Promise.all(keys.map(k=>caches.delete(k))));this.toast('تم تصفير إعدادات الواجهة. ستُحدّث الصفحة الآن','success');setTimeout(()=>location.reload(),900)};
  const base=A.showSection.bind(A);
  A.showSection=async function(section){if(section==='maintenance_center'){this.currentSection=section;document.querySelectorAll('.nav-item').forEach(el=>el.classList.toggle('active',el.dataset.section===section));const area=document.getElementById('content-area');document.getElementById('page-title').textContent='مركز الصيانة';return this.renderMaintenanceCenter(area)}return base(section)};
})();
