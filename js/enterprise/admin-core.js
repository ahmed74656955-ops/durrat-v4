...
const AdminApp = {
  ...
  async saveBranch(button) {
    ...
    try {
      const exists = await supabase.from('branches').select('code').eq('code', code).maybeSingle();
      if (exists.error) throw exists.error;
      if (exists.data) throw new Error('كود الفرع موجود مسبقًا. اختر كودًا آخر.');

      try {
        await this.rpcJson('create_branch_v2', { p_code: code, p_name: name, p_start_seq: seq });
      } catch (rpcError) {
        const { error } = await supabase.from('branches').insert([{ code, name, start_seq: seq, is_active: true }]);
      ...
      this.toast('فشل حفظ الفرع: ' + this.dbMessage(error), 'error');
  ...
  async saveUser(button) {
    ...
    this.setBusy(button, true);
    try {
      const exists = await supabase.from('app_users').select('id, username').eq('username', u).maybeSingle();
      if (exists.error) throw exists.error;
      if (exists.data) throw new Error('اسم المستخدم موجود مسبقًا. لا يمكن إنشاء مستخدمين بنفس الاسم.');

      try {
        await this.rpcJson('create_app_user_v2', { p_branch_code: b, p_username: u, p_password: p, p_role: r });
      } catch (rpcError) {
        ...
        if (/exists|already|duplicate|موجود/i.test(resultText)) throw new Error('اسم المستخدم موجود مسبقًا');
      }
      ...
      this.toast('فشل حفظ المستخدم: ' + this.dbMessage(error), 'error');
    } finally { this.setBusy(button, false); }
  },
  ...
  async toggleUserStatus(id, status) {
    try {
      ...
      this.showSection('users');
    } catch (error) { this.toast('فشل تغيير حالة المستخدم: ' + this.dbMessage(error), 'error'); }
  },
  ...
  async toggleBranchStatus(id, status) {
    try {
      ...
      this.showSection('branches');
    } catch (error) { this.toast('فشل تغيير حالة الفرع: ' + this.dbMessage(error), 'error'); }
  },


  dbMessage(error) {
    const raw = String(error?.message || error || '');
    if (/app_users_username_key|duplicate key|unique/i.test(raw)) return 'اسم المستخدم موجود مسبقًا. اختر اسمًا آخر مثل ali_main أو branch01.';
    if (/branches_code_key|branch.*unique/i.test(raw)) return 'كود الفرع موجود مسبقًا. اختر كودًا آخر.';
    if (/violates foreign key|foreign key/i.test(raw)) return 'الفرع المحدد غير موجود أو غير نشط.';
    if (/permission denied|42501/i.test(raw)) return 'لا توجد صلاحية كافية للحفظ. شغّل ملف SQL المرفق أولًا.';
    return raw || 'حدث خطأ غير معروف';
  },

  async rpcJson(name, args) {
    const { data, error } = await supabase.rpc(name, args);
    if (error) throw error;
    if (data && typeof data === 'object' && data.success === false) throw new Error(data.error || 'فشلت العملية');
    return data;
  },
